   ...
   <div class="container-fluid">
      <div class="row">
         ...
         <div class="col-md-2 p-2">
            <div class="row pt-3 menu-add-class">
               ...
               <div class="col-md-6">
                  ...
                  <form id="logout-form" action="{{ route('website.logout') }}" method="POST" class="display-none">
                     @csrf
                  </form>
               </div>
               ...
               <div class="col-md-12 text-center bottom-position">
                  <img src="{{asset('assets/website/images/bottom-img.png')}}" alt="" class="img-fluid w-75 mx-auto d-block">
                        ...
                        <ul class="nav nav-pills mb-3 nav-category" id="pills-tab">
                           <li class="nav-item">
                              <button class="nav-link catg-link active d-flex align-items-center rounded-pill px-4 get_menu1" id="pills-all" data-bs-toggle="pill" data-bs-target="#all" type="button">
                                 <img src="{{asset('assets/website/images/1.webp')}}" alt="" style="width:40px;height: 40px;border-radius: 50%;" class="me-4"> All</button>
                           </li>
                           @foreach ($category as $item)
                           <li class="nav-item">
                              <button class="nav-link catg-link d-flex align-items-center rounded-pill px-4 get_menu" data-id="{{$item->id}}" id="pills-all" data-bs-toggle="pill" data-bs-target="#all">
                                 <img src="{{url($item->cate_img)}}" alt="" style="width:40px;height: 40px;border-radius: 50%;" class="me-4">{{$item->cate_name}}</button>
                           </li>
                           @endforeach
                        </ul>
                                 ...
                                 <div class="contact-form">
                                    <form method="POST" class="update_profile">
                                       <div class="row">
                                          <div class="col-md-6">
                                             <div class="mb-3">
                                                <input type="text" class="form-control" placeholder="Name*" name="name" value="@if(!empty(Auth::user()->name)) {{Auth::user()->name}} @endif">
                                             </div>
                                          </div>
                                          <div class="col-md-6">
                                             <div class="mb-3">
                                                <input type="email" class="form-control" placeholder="Email*" name="email" value="@if(!empty(Auth::user()->email)) {{Auth::user()->email}} @endif">
                                             </div>
                                          </div>
                                          <div class="col-md-6">
                                             <div class="mb-3">
                                                <input type="date" class="form-control" placeholder="Date of Birth*(m-d-Y)" name="dob" value="@if(!empty(Auth::user()->dob)) {{date('dd-mm-yyyy' , strtotime(Auth::user()->dob))}} @endif">
                                             </div>
                                          </div>
                                          <div class="col-md-6">
                                             <div class="mb-3">
                                                <select name="age" id="age-group" class="form-control">
                                                   ...
                                                   <option value="10-25" @if(!empty(Auth::user()->age)) {{Auth::user()->age == "10-25" ? 'selected' : '' }} @endif>10 - 25</option>
                                                   <option value="15-25" @if(!empty(Auth::user()->age)) {{Auth::user()->age == "15-25" ? 'selected' : '' }} @endif>15 - 25</option>
                                                   <option value="20-25" @if(!empty(Auth::user()->age)) {{Auth::user()->age == "20-25" ? 'selected' : '' }} @endif>20 - 25</option>
                                                </select>
                                             </div>
                                          </div>
         ...
         <div class="col-md-3 mt-2">
            ...
            <!-- Clearfix 2-->
            <div id="coupon-hide" class="mt-3" style="display: flex;justify-content: space-evenly;align-items: stretch;">
               <input type="text" class="form-control-sm w-100" placeholder="Apply Coupon" id="apply_coupon"><button class="btn btn-danger" id="coupon_apply">Apply</button>
            </div>
            ...
            <div class="row mt-2">
                <div class="col-md-6">
                    <div class="form-check">
                        <input type="radio" class="form-check-input hmd" id="hmd" value="Home Delivery" name="home_delivery">
                ...
                <div class="col-md-6">
                    <div class="form-check">
                        <input type="radio" class="form-check-input ta" id="ta" value="Take Away" name="home_delivery">
...
<div class="modal fade" id="pizzaSidesModal" tabindex="-1">
    <div class="modal-dialog modal-sm modal-dialog-centered modal-sm-mid">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel" style="font-size: 14px">Select the side you want your toppings</h5>
{{--                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>--}}
            </div>
                        ...
                        <div class="card">
                            <div class="card-body display-flex-space-between">
                                <div class="text-center">
                                    <input class="toppingdirection" id="left" type="radio" value="left" name="spreadPart" />
                                    <label class="avatar-container p--50" for="left"></label>
                                    <p><span>Left side</span></p>
                                </div>
                                <div class="text-center">
                                    <input class="toppingdirection" id="whole" type="radio" value="whole" checked name="spreadPart" />
                                    <label class="avatar-container p-100" for="whole"></label>
                                    <p><span>Whole pizza</span></p>
                                </div>
                                <div class="text-center">
                                    <input class="toppingdirection" id="right" type="radio" value="right" name="spreadPart" />
                                    <label class="avatar-container p-50" for="right"></label>
                                    <p><span>Right side</span></p>
                                </div>
                                <div class="text-center">
                                    <input class="toppingdirection" id="quarter" type="radio" value="quarter" name="spreadPart" />
                            ...
                            <div class="card-fotter text-center">
                                <button class="btn btn-success selectSidesButton" style="margin-bottom: 4%;padding: 7px 25px;">OK</button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).on('change', '.abc', function(e){
        ...
        if($(this).is(':checked'))
        {
            ...
            $('#pizzaSidesModal').modal({backdrop: 'static', keyboard: false});
    ...
    $(function(){
        ...
        $('.selectSidesButton').click(function(e){
            ...
            if($('.toppingdirection:checked').val() == 'left'){
                $('.appendSide-'+property_id).html(`<div class="avatar-container p--50"></div><input type="hidden" name="p-${property_id}" value="left">`)
            }else if($('.toppingdirection:checked').val() == 'right'){
                $('.appendSide-'+property_id).html(`<div class="avatar-container p-50"></div><input type="hidden" name="p-${property_id}" value="right">`)
            }else if($('.toppingdirection:checked').val() == 'quarter'){
                $('.appendSide-'+property_id).html(`<div class="avatar-container p-25"></div><input type="hidden" name="p-${property_id}" value="quarter">`)
            }else{
                $('.appendSide-'+property_id).html(`<div class="avatar-container p-100"></div><input type="hidden" name="p-${property_id}" value="whole">`)
            }
...
<script>
   ...
   $("body").on("click", ".topping_updated", function(e) {
      e.preventDefault();
      var id = $(this).data('id');
      ...
      $.ajax({
         url: "{{ route('website.extra_variants_update') }}",
         type: "POST",
         data: fd,
         dataType: 'json',
         processData: false,
               ...
               if (data.products.length > 0) {
                  $.each(data.products, function(i, val) {
                     if (val.discount) {
                        ...
                        var discount = '<strike><span class="text-danger">$' + parseFloat(val.price).toFixed(2) + '</span></strike><span> <br> <span><b>Discount&nbsp</b>' + dis + '%</span>'
                        var multipleDiscount = '<strike><span class="text-danger">$' + parseFloat(val.price).toFixed(2) + '</span></strike>'
                        var discountPercent = '<span>  <span><b>Discount&nbsp</b>' + dis + '%</span>'
                     } else {
                        var dis = 0;
                        var price = (val.price)
                        var discount = ''
                     ...
                     if(val.hasDefaultVarient){
                         var custom = '<a href="" data-id="' + val.id + '" class="topping_added"><i class="fa fa-cutlery"></i> Customize</a>'
                         var add = '<a class="p-2 btn-primary topping_added" data-id="' + val.id + '"><i class="fa fa-cart-shopping"></i> Add</a>'
                         if (val.discount) {
                             var discountedPrice = '<strike><span class="text-danger">$' + parseFloat(val.total_default_price).toFixed(2) + '</span></strike>'
                             var discpuntedMprice = val.total_default_price-((dis/100)*val.total_default_price)
                         }else{
                             var discountedPrice = ''
                             var discpuntedMprice = val.total_default_price
                         }
                         var priceC = '<b><span><span class="text-success">$' + parseFloat(discpuntedMprice).toFixed(2) + '</b></span>&nbsp;&nbsp;'+discountedPrice+'<br>'
                         ...
                         if(val.hasVarient){
                             ...
                             $.map(val.allVarients, function (value, index){
                                 if (val.discount) {
                                     var discountedPrice = '<strike><span class="text-danger">$' + parseFloat(value.price).toFixed(2) + '</span></strike>'
                                     var discpuntedMprice = value.price-((dis/100)*value.price)
                                 }else{
                                     var discountedPrice = ''
                                     var discpuntedMprice = value.price
                                 }
                                 priceC += '<b><span class="text-muted">'+value.name+'</span> : <span><span class="text-success">$' + parseFloat(discpuntedMprice).toFixed(2) + '</b></span>&nbsp;&nbsp;'+discountedPrice+'<br>'
                             })
                             priceC += discountPercent
                             var sty = 'style="font-size: 12px;line-height: 16px;"'
                         }else{
                             var custom = ''
                             var add = '<a class="p-2 btn-primary add_to_cart" data-discount_percent="'+dis+'" data-normal_price="'+parseFloat(val.price).toFixed(2)+'" data-discounted_price="'+parseFloat(price).toFixed(2)+'" data-id="' + val.id + '"><i class="fa fa-cart-shopping"></i> Add</a>'
                             var priceC = '<b><span><span class="text-success">$' + parseFloat(price).toFixed(2) + '</b></span>&nbsp;&nbsp;'+discount;
                             var sty = '';
                         }
                     }
                     var url = "{{ url('')}}" + val.product_img;
                     datas += '<div class="col-xl-4 col-lg-6 col-md-12">' +
                        '<div class="card card-menu mb-3" style="height: 335px">' +
                        '<div class="card-body">' +
                        '<img src="' + url + '" alt="" class="img-menu mx-auto d-block">' +
                        '<p class="text-center fw-bold mt-3 mb-0">' + val.product_name + '</p>' +
                        '<p class="text-center" '+sty+'>'+priceC+'</p>' +
               ...
               if (data.products.length > 0) {
                  $.each(data.products, function(i, val) {
                     console.log(val.image);
                      if (val.discount) {
                          if(val.hasVarient){
                              ...
                              var dis = val.discount;
                              var price = (val.grandTotal - (val.grandTotal * val.discount / 100));
                          }
                      } else {
                          var dis = 0;
                          var price = val.grandTotal;
                     ...
                     if(val.type == "combo") {
                      ...
                      if(val.hasVarient){
                          var varient = '<br><span><b>Size : </b>' + val.varientName + '</span>'
                      }else{
                          var varient = ''
                      }
                     ...
                     datas += ' <div class="card mb-3 main-cart pb-3 mt-0">' +
                        ...
                        '<p class="card-text mb-0"><span class="text-success"><b>$' + price + '</b>'+varient+Toppings+Properties+'' +
                        '</div>' +
                        '</div>' +
                        '<div class="col-md-2">' +
                        '<div class="qty-block">' +
                  ...
                  $.each(data.products, function(i, val) {
                     ...
                     if(val.hasVarient){
                      ...
                      if(val.hasProperties){
                          ...
                          $.map(val.propertyItems, function (elem, index) {
                              Properties += '<p style="display: flex; align-items: center;justify-content: space-between;padding: 3% 18% 0% 0%;">' +elem.name + '</p>'
                          })
                          Properties += '</div>'
                      }else{
                          var Properties = ''
                     ...
                     datas += '<div class="card mb-3 bg-cart pb-3">' +
                        ...
                        '<a href="javascript:void(0)" class="remove_to_cart" data-id="' + val.id + '"><i class="fa fa-close  me-2"></i>' +
                        '</p>' +
                        '</div>' +
                        ...
                        '<p class="card-title mb-0 fw-bold n-size">' + name + '</p></a>' +
                        '<p class="card-text mb-0"><span class="text-success"><b>$' + price + '</b>'+varient+Toppings+'</p>' +
                        '</div>' +
                        '</div>' +
                        ...
                        '</div>' +
                        '</div>' +
                        '<div class="offset-xl-3 col-xl-9 col-md-12">'+Properties+'</div>' +
                        '<a data-id=' + val.product_id + ' data-orderid=' + val.id + ' class="topping_updated "><button class="btn btn-primary w-100">Update Your Order</button></a>'+
                        '</div>' +
                        '</div>';
                  })
               } else {
               ...
               // $("#total").text(parseFloat(net_amnt).toFixed(2))
                if (typeof data.deliveryCharges.hasDeliveryCharges != "undefined") {
                    if(data.deliveryCharges.hasDeliveryCharges){
                        ...
                        var priceAfterDelivery = parseFloat(net_amnt) + parseFloat(delivery_chagre)
                        $("#total").text(parseFloat(priceAfterDelivery).toFixed(2))
                        $(".total").val(parseFloat(priceAfterDelivery).toFixed(2))
                    }else{
                        // alert('no')
                        $('.delivery_inc_show').hide()
                        $("#total").text(parseFloat(net_amnt).toFixed(2))
   ...
   $("body").on("submit", ".new_add_to_cart", function(e) {
    ...
    // Extra add to cart
    $("body").on("submit", ".update_cart", function(e) {
      e.preventDefault();
      let fd = new FormData(this)
      fd.append('_token', "{{ csrf_token() }}");
      $.ajax({
         url: "{{ route('website.update_cart') }}",
         type: "POST",
         data: fd,
         dataType: 'json',
         processData: false,
      ...
      $.ajax({
         ...
         success: function(result) {
            if (result.data) {
               ...
               $('#coupon-hide').hide()
                $('#coupon_selected').html('<input class="coupon_code" type="hidden" name="coupon_code" value="'+value+'">')
            } else {
